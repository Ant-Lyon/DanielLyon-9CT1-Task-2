...
|---------- |---------- |----------------   |
| Laser beam untouched | Light detector continuously receives the laser light | System waits and does not trigger the buzzer     |
| Intruder enters rooms | Laser beam is broken; light detector is triggered | Buzzer activates instantly to alert user. |

This worked very well, but only when I used a 10 KΩ resistor for my voltage divider. We don't really know why it works except that a ratio is made by the two resistors, but Daniel Lyon's Dad who is an electrician said you don't need to know exactly HOW it works, just that it does, and how use it. The 10 KΩ resistor meant we had the maximum u16 range, with at dark light levels averaged 1500 but with the laser shining into it at around 60000. Surprisingly, even in dark and very bright areas when the laser shone at the LDR the voltage would be 60000, then in light areas drop to around 50000. This made us toggle the system so that when the LDR detectors a light level of under 55000, it will detect.
...
The five-second interval was repeatably noted as "too long", however the video demonstration which we showed them did not have the updated time.sleep() where the interval is shorter if the buzzer played just before. But we could reduce the interval to maybe 3 seconds, which would also mean no interval if the buzzer played since it takes 3 seconds to play. If we had a Pico W (the one with Wi-Fi capabilities), we could just connect to a server on the internet using the 'ntptime' library with the 'network' library. But that is out of scope for this project due to the insufficient hardware capabilities. Perhaps we could add a two-colour LED to display RED when the door is closed, and GREEN when it is open to represent whether the laser is primed. This would make it easier to know whether the system thinks the door is open or not when it is slightly ajar without looking for the red glow surrounding the laser module or the red dot on the LDR.
