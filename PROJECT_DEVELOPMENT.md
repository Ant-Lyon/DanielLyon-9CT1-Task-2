...
The five second interval was repeatably noted as "too long", however the video demonstration which we showed them did not have the updated time.sleep() where the interval is shorter if the buzzer played just before. But we could reduce the interval to maybe 3 seconds, which would also mean no interval if the buzzer played since it takes 3 seconds to play. If we had a Pico W (the one with wifi capabilities), we could just connect to a server on the internet using the 'ntptime' library with the 'network' library. But that is out of scope for this project due to the insufficient hardware capabilities. Perhaps we could add a two-colour LED to display RED when the door is closed, and GREEN when it is open to represent whether or not the laser is primed. This would make it easier to know whether or not the system thinks the door is open or not when it is slightly ajar without looking for the red glow surrounding the laser module or the red dot on the LDR.
